...
<body id="page-top" data-spy="scroll" data-target=".navbar-fixed-top" style="background-color: #FFFFFF">
	...
	<?php
		$id = $_GET['id'];
		$value = $_GET['value'];
		$sql = mysqli_query($con, "SELECT * FROM `alunos_pendentes` WHERE id = $id");
		$row = $sql->fetch_assoc();
		...
		$nivel=$row['nivel'];
		$grau="Bacharelado";
		$ingresso=$row['ingresso'];//mudei essa linha
		$turno="Diurno";
		...
		if($value == "1")
		{
			$sql = mysqli_query($con, "INSERT INTO `alunos` (`nome`, `sobrenome`, `data_de_nascimento`, `email`, `cpf`, `senha`, `status`, `curso`, `data_de_cadastro`, `matricula`, `grau`, `nivel`, `identidade`, `orgao_expeditor`, `turno`, `ingresso`, `endereco`, `cep`) VALUES ('$nome', '$sobrenome', '$data', '$email', '$cpf', '$senha', '$status', '$curso', '$cadastro', '$matricula', '$grau', '$nivel', '$identidade', '$orgao', '$turno', '$ingresso', '$endereco', '$cep')");
			...
			if($sql)
			{
				...
				echo "</script>";	
				//deleta
				$sql = mysqli_query($con, "DELETE FROM `alunos_pendentes` WHERE `id` = $id"); //Não exclui ;-; - Agora exclui por id
			}
			else{
				echo "<script type='text/javascript'>";
		...
		else if($value == "0") //email dizendo que o cadastro foi rejeitado
		{
			...
			echo "</script>";	
			$sql = mysqli_query($con, "DELETE FROM `alunos_pendentes` WHERE `id` = $id"); 
		}
...
$body .= $message.$eol;
	...
	if(send_email)	// Caso tenha inserido com sucesso
		$envio = mail($to, "Solicitação de cadastro", $body, $headers);

		?>

		<script>
			window.location.replace("aprovar-cadastro-aluno.php"); //substituindo o header
		</script>
