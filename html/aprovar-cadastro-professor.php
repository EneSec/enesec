<?php
	...
	if($_SESSION['Permissao'] != "admin" && $_SESSION['Permissao'] != "secretaria") {
		include('logout.php');
	}

	$listaSQL = "SELECT * FROM `professores_pendentes` ORDER BY `id`";
...
<html lang="en">
    ...
    <br><br>
		...
		<h1 class="text-info text-center"> Cadastro de professores(as) </h1>

		<div class="container" style="color: #000000">
			<table id="professor" class="table table-hover" style="color: #000000">
				<thead id="header">
					<tr class="info">
						<th> Nome </th>
						<th> Matrícula </th>
					...
					<?php 
						if($rs->num_rows > 0) {
							while($row = $rs->fetch_assoc()) {
								...
								echo "<td id='fileira'>" .$row['tipo'] ."</td>";
			...
			<div class="container text-center">
				...
				<?php
					if($_SESSION['Permissao'] == "secretaria") {
						echo "<input type = 'button' class='btn btn-danger' value = 'Voltar' onclick = 'retornar_sec()'>";
					}

					else if($_SESSION['Permissao'] == "admin") {
						echo "<input type = 'button' class='btn btn-danger' value = 'Voltar' onclick = 'retornar_admin()'>";
					}
				?>
		...
		<script>
			...
			function confirmar(id) {
				var redirect = 'cadastrar-professor.php?id=' + id + '&value=1';
				window.location.href = redirect;
			}

			function rejeitar(id) {
				var redirect = 'cadastrar-professor.php?id=' + id + '&value=0';
