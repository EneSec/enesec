...
<body id="page-top" data-spy="scroll" data-target=".navbar-fixed-top" style="background-color: #FFFFFF">
	...
	$id = $_GET['id'];
	$value = $_GET['value'];
		$sql = mysqli_query($con, "SELECT * FROM `secretaria_pendentes` WHERE id = $id");
		$row = $sql->fetch_assoc();
		...
		if($value == "1")
		{
			$insert = "INSERT INTO `secretaria` (`nome`, `sobrenome`, `data_de_nascimento`, `email`, `cpf`, `senha`, `data_de_cadastro`, `identificador`, `identidade`, `orgao_expeditor`, `endereco`, `cep`) VALUES ('$nome', '$sobrenome', '$data', '$email', '$cpf', '$senha', '$cadastro', '$matricula', '$identidade', '$orgao', '$endereco', '$cep')";
			...
			if($sql)
			{
				...
				echo "</script>";	
				//deleta
				$sql = mysqli_query($con, "DELETE FROM `secretaria_pendentes` WHERE `id` = $id"); //Não exclui ;-; - Agora exclui por id
			}
			else{
				echo "<script type='text/javascript'>";
		...
		else if($value == "0") //email dizendo que o cadastro foi rejeitado
		{
			...
			echo "</script>";
			$sql = mysqli_query($con, "DELETE FROM `secretaria_pendentes` WHERE `id` = $id"); 
		}
...
$body .= $message.$eol;
		...
		<script>
			window.location.replace("aprovar-cadastro-secretaria.php"); //substituindo o header
		</script>
